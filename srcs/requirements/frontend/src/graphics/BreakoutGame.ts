...
export class BreakoutGame {
  ...
  private scoreCallback:
    | ((score: number, level: number, lives: number) => void)
    | null = null;
  ...
  private createBricks(): void {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const z =
          (row - rows / 2) * (brickHeight + padding) +
          brickHeight / 2 +
          zOffset;
        const y = 0;
  ...
  public moveSecondPaddleLeft(): void {
    // For PvP mode - move second paddle left
    console.log("Moving second paddle left");
  }

  public moveSecondPaddleRight(): void {
    // For PvP mode - move second paddle right
    console.log("Moving second paddle right");
  }

  public stopSecondPaddle(): void {
    // For PvP mode - stop second paddle
    console.log("Stopping second paddle");
  }
  ...
  public setGameMode(
    ...
    console.log(
      `Setting game mode to: ${mode}, difficulty: ${difficulty || "medium"}`
    );
  }

  public setScoreCallback(
    callback: (score: number, level: number, lives: number) => void
  ): void {
  ...
  private spawnPowerUp(position: BABYLON.Vector3): void {
    ...
    const types: Array<"expand-paddle" | "multi-ball" | "slow-ball"> = [
      "expand-paddle",
      "multi-ball",
      "slow-ball",
    ];
  ...
  private checkGameOver(): void {
    ...
    if (allBricksDestroyed) {
      ...
      // Check if player has completed all levels (let's say 5 levels for now)
      if (this.level > 5) {
        ...
        this.showNotification(
          `Complimenti! Hai completato tutti i livelli! Punteggio finale: ${this.score}`,
          "success"
        );
      }
      ...
      if (this.lives <= 0) {
        ...
        this.showNotification(
          `Game Over! Punteggio finale: ${this.score}`,
          "error"
        );
      } else {
        // Reset ball position
