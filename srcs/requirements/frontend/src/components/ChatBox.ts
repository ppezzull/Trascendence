...
export class ChatBox {
  ...
  private handleWebSocketMessage(message: ChatMessage) {
    if (this.currentThreadId && message.threadId === this.currentThreadId) {
      ...
      // Check if this message already exists (it might be a message we sent)
      const existingMessageIndex = this.messages.findIndex(
        (msg) => msg.id === transformedMessage.id
      );

      if (existingMessageIndex >= 0) {
        // Update the existing message (e.g., fix the sender name)
        this.messages[existingMessageIndex] = transformedMessage;
      } else {
        // Add new message
  ...
  private async loadMessages(threadId?: number) {
    console.log("threadIdensommamammamamammama", threadId);
    try {
      if (threadId) {
        const response = await this.apiService.getChatMessages(
          threadId.toString()
        );

        console.log("responseensommamammamamammama", response);

        const nameSender = await this.apiService.getUserById(
          response.sender_id
        );

        console.log("nameSenderensommamammamamammama", nameSender);

        if (response && Array.isArray(response)) {
          const messages = response.map(async (msg: any) => {
            const nameSender = await this.apiService.getUserById(msg.sender_id);
          ...
          this.messages = [
            {
              ...
              senderName: "System",
              content:
                "Chat iniziata! Invia un messaggio per iniziare la conversazione.",
              timestamp: new Date(),
              threadId: threadId,
              isSystem: true,
            },
        ...
        this.messages = [
          {
            ...
            senderName: "System",
            content:
              "Benvenuto nella chat cyberpunk! Seleziona un utente per iniziare a chattare.",
            timestamp: new Date(),
            threadId: 0,
            isSystem: true,
          },
  ...
  private renderUsers() {
    console.log(
      "renderUsers called, _chatThreads length:",
      this._chatThreads.length
    );
    ...
    if (this._chatThreads.length === 0) {
      chatThreadsList.innerHTML =
        '<div class="text-cyber-green/50 text-sm text-center py-4">Nessuna chat trovata</div>';
      return;
    }

    chatThreadsList.innerHTML = this._chatThreads
      .map((thread) => {
        // Find other participant (not current user)
        const otherParticipantId = thread.members.find(
          (id: number) => id !== this.currentUserId
        );
  ...
  private renderMessages() {
    ...
    console.log("messagesensommamammamamammama", this.messages);
    ...
    this.messageContainer.innerHTML = this.messages
      ...
      <div class="flex ${
        ...
        <div class="${
          message.isSystem
            ? "bg-cyber-dark/50 text-cyber-cyan text-xs italic px-2 py-1 rounded"
            : message.senderId === this.currentUserId?.toString()
            ? "cyber-panel max-w-xs"
            : "cyber-card max-w-xs"
        }">
          ${
            !message.isSystem
  ...
  private async sendMessage() {
    ...
    try {
      ...
      if (response && (response.success || response)) {
        ...
        // The response might be the message object directly or wrapped in a data property
        const messageData = response.data || response;
        const messageId = messageData.id?.toString() || Date.now().toString();

        const newMessage: Message = {
          id: messageId,
          senderId: this.currentUserId?.toString() || "",
          senderName: "Me", // This will be updated by the WebSocket response
          content: content,
          timestamp: new Date(),
  ...
  private async inviteToGame(userId: string) {
    ...
    try {
      ...
      const response = await this.apiService.sendGameInvitation(
        userId,
        "default"
      );
      ...
      if (response.success) {
        this.showNotification(
          `Invito di gioco inviato a ${user.username}`,
          "success"
        );
  ...
  private renderSearchResults() {
    ...
    searchResultsElement.innerHTML = this.searchResults
      .map(
        (user) => `
      <div class="cyber-card p-2 cursor-pointer hover:border-cyber-cyan" data-search-user-id="${
        user.id
      }">
  ...
  private async loadChatThreads() {
    try {
      ...
      if (response) {
        ...
        console.log(
          "Chat threads data set to _chatThreads:",
          this._chatThreads
        );
        console.log(
          "_chatThreads length after setting:",
          this._chatThreads.length
        );
        ...
        setTimeout(() => {
          console.log(
            "_chatThreads length after 100ms:",
            this._chatThreads.length
          );
        }, 100);
        ...
        for (const thread of this._chatThreads) {
          ...
          if (
            otherParticipantId &&
            !this.threadParticipants.has(otherParticipantId)
          ) {
            try {
              // Get user info for other participant
              const userResponse = await this.apiService.getUserById(
                otherParticipantId.toString()
              );
              console.log(
                "User response for participant",
                otherParticipantId,
                ":",
                userResponse
              );
              if (userResponse.success && userResponse.data) {
                this.threadParticipants.set(
                  otherParticipantId,
                  userResponse.data
                );
              }
        ...
        console.log(
          "About to call renderUsers, _chatThreads length:",
          this._chatThreads.length
        );
      ...
      if (thread) {
        ...
        if (otherParticipantId) {
          const participantInfo =
            this.threadParticipants.get(otherParticipantId);
          const displayName = participantInfo
            ? participantInfo.display_name || participantInfo.username
            : `Utente ${otherParticipantId}`;
